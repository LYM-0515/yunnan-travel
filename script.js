...
avatarFrame.addEventListener('click', () => avatarInput.click());
...
function renderPhotos() {
  ...
  photos.forEach((dataUrl, index) => {
    ...
    card.innerHTML = '<img src="' + dataUrl + '" alt="照片 ' + (index + 1) + '"><button class="photo-delete" data-index="' + index + '">&times;</button>';
...
modalOverlay.innerHTML = '<div class="modal"><button class="modal-close">&times;</button><img src="" alt="照片预览"></div>';
...
// ===================== City Photo Upload (with event delegation) =====================
const aboutGrid = document.getElementById('aboutGrid');

function handleCityPhotoChange(e) {
  const file = e.target.files[0];
  if (!file) return;
  ...
  reader.onload = (ev) => {
    ...
    if (placeholder) {
      placeholder.innerHTML = '<img src="' + ev.target.result + '" alt="' + city + '" style="width:100%;height:100%;object-fit:cover;display:block;">';
      placeholder.style.border = 'none';
      placeholder.style.background = 'none';
    }
    const allCityPhotos = {};
    document.querySelectorAll('.about-img-placeholder img').forEach(img => {
      const p = img.closest('.about-img-placeholder');
      if (p && p.dataset.city) allCityPhotos[p.dataset.city] = img.src;
    });
    try { localStorage.setItem('cityPhotos', JSON.stringify(allCityPhotos)); } catch(err) {}
  };
  reader.readAsDataURL(file);
}

// Setup each city's file input
document.querySelectorAll('.city-photo-input').forEach(input => {
  input.addEventListener('change', handleCityPhotoChange);
});

// Click placeholder -> trigger file input (event delegation on the grid)
aboutGrid.addEventListener('click', function(e) {
  const placeholder = e.target.closest('.about-img-placeholder');
  if (!placeholder) return;
  const city = placeholder.dataset.city;
  if (!city) return;
...
(function() {
  const saved = localStorage.getItem('cityPhotos');
  if (!saved) return;
  try {
    ...
    Object.keys(photos).forEach(city => {
      const placeholder = document.querySelector('.about-img-placeholder[data-city="' + city + '"]');
      if (placeholder) {
        placeholder.innerHTML = '<img src="' + photos[city] + '" alt="' + city + '" style="width:100%;height:100%;object-fit:cover;display:block;">';
        placeholder.style.border = 'none';
        placeholder.style.background = 'none';
      }
    });
